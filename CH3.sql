...
/*3.1 Retrieve the account id, customer id, and available balance for all accounts whose status equals 'ACTIVE' and whose available balance is greater than 2500*/
select account_id, cust_id, avail_balance from account where status='ACTIVE' and avail_balance > 2500;
/*3.1 Write a query against the account table that retruns the IDs of the employees whho opened the accounts (use the account.open:emp:id column). Include a single row
for each distinct employee*/
select distinct(open_emp_id) from account;
/*3.1 Fill in the blanks (denoted by <#>) for this multidataset query to achieve the results shown here*/
